  ...
  "items": [
    {
      ...
      "unit_price": "ราคาต่อหน่วย (บาท) ที่พิมพ์ในคอลัมน์ 'หน่วยละ' ของบรรทัดนี้ (number | null) — อ่านเพื่อ **ตรวจทานจำนวนเท่านั้น**; **ห้ามใช้ตัวเลขขนาดบรรจุ** เช่น '1 ลัง @12' หรือ '12 ขวด/ลัง' (12 คือจำนวนขวด ไม่ใช่ราคา) ถ้าไม่มีคอลัมน์ราคาชัดเจนใส่ null"
    }
  ],
  "confidence": 0.85,
...
- **สองฟิลด์หลักของแต่ละ item**:
  - `product_name_raw` = ข้อความต้นฉบับจากเอกสาร ตรงตามตัวอักษร/ลายมือที่อ่านได้
    ...
    - ทำได้แค่: ตัด **หน่วยนับ** ที่ตามหลังตัวเลข เช่น "2 ลัง"/"×12 ขวด"/"3 แพ็ค" และ **return marker** เช่น "เปลี่ยน/ถาด" ออก
    - **ห้ามตัด** คำบรรจุภัณฑ์ที่ติดกับชื่อสินค้า เช่น "เบียร์สิงห์กระป๋อง", "ลีโอขวดใหญ่", "น้ำสิงห์ขวดแก้ว" —
      คำว่า กระป๋อง/ขวดใหญ่/ขวดเล็ก/ขวดแก้ว ตรงนั้นคือ **ตัวแยก SKU** ต้องคงไว้ใน raw ทั้งคำ
    - ตัวอย่าง: receipt = "SODAPP โซดาเปลี่ยน/ถาด ×12"  →  raw = "SODAPP โซดา"
    - ตัวอย่าง: receipt = "เบียร์สิงห์กระป๋อง  12 ลัง"  →  raw = "เบียร์สิงห์กระป๋อง" (ตัดแค่ "12 ลัง")
...
- **น้ำดื่มสิงห์ — เลือกขนาดให้ถูก** : catalog มีน้ำสิงห์ 3 SKU (ขวดแก้วเปลี่ยนขวด, PET 600ml, PET 1.5L)
  - ถ้า raw ระบุขนาด → เลือกตามขนาด: "น้ำสิงห์ 600"/"600cc" → "น้ำสิงห์เพ็ท 600ml", "น้ำสิงห์ 1.5"/"1.5L" → "น้ำสิงห์เพ็ท 1.5L"
  - ถ้า raw ระบุ "ขวดแก้ว"/"เปลี่ยนขวด"/"คืนขวด" → "น้ำดื่มสิงห์ขวดแก้ว"
  - **ถ้า raw เป็น "น้ำสิงห์" เฉยๆ ไม่ระบุขนาด/บรรจุภัณฑ์ → default เป็น "น้ำดื่มสิงห์ขวดแก้ว"** (ตัวที่ร้านโชห่วยสั่งเป็นลังประจำ)
  - **ระวังอ่านผิด (สำคัญ)**: บรรทัดที่ขึ้นต้นด้วย "น้ำ" (น้ำสิงห์ / น้ำดื่ม) = **น้ำดื่ม** เสมอ
    **ห้าม** map เป็น "สิงห์เลมอนโซดา" หรือ "โซดาสิงห์" แม้ลายมือจะคล้ายหรืออ่านยาก —
    "น้ำสิงห์" (น้ำเปล่า) กับ "สิงห์เลมอนโซดา"/"โซดาสิงห์" (น้ำอัดลม) เป็นคนละสินค้า
    ให้ดูคำแรกของบรรทัดให้ชัด: ขึ้นต้น "น้ำ…" → น้ำดื่ม, ขึ้นต้น "เลมอน…/โซดา…" → น้ำอัดลม
  - **บิลเดียวกันมักลง SKU ละ 1 บรรทัด**: ถ้าจะให้ 2 บรรทัดได้ `product_code` เดียวกัน
    แต่คอลัมน์ "หน่วยละ" ต่างกัน (เช่น 155 กับ 85) → เกือบแน่นอนว่าเป็น **คนละสินค้า** ให้กลับไปอ่านคำแรก
    ของทั้งสองบรรทัดใหม่ (โซดาสิงห์ กับ น้ำสิงห์ ที่ลงท้าย "เปลี่ยนขวด" เหมือนกัน เป็นคู่ที่สับสนบ่อย)
    และอย่าเดา SKU ให้ต่างกันโดยไม่มีตัวอักษรรองรับ
- **คำต่อท้ายสินค้าที่ไม่ใช่ส่วนของชื่อ** : คำเหล่านี้อธิบาย **รูปแบบการขาย/การบรรจุ** ไม่ใช่ชื่อสินค้า
  ...
  - "ลัง" / "กระป๋อง" / "ขวด" / "แพ็ค" **ที่ตามหลังตัวเลขจำนวน** → เป็น **unit** ให้ใส่ใน field `unit` ไม่ใช่ชื่อสินค้า
    (แต่ถ้าติดกับชื่อ เช่น "สิงห์กระป๋อง" / "ลีโอขวดใหญ่" = รูปแบบบรรจุที่แยก SKU → คงไว้ใน raw และใช้เลือก product_code)
